...
MATCH_SUBSTRING=0
[[ "$FICHIER_URLS" == "kr" ]] && MATCH_SUBSTRING=1
...
generer_kwic_tsv() {
  ...
  local w="$5"
  local substring="${6:-0}"  # 1 => substring (kr), 0 => exact/prefix (be)

  perl -Mutf8 -CS -e '
    use strict; use warnings;
    use Encode qw(decode FB_DEFAULT FB_QUIET);

    my ($txt, $out, $label, $joined, $w, $substring) = @ARGV;

    $label  = decode("UTF-8", $label,  FB_DEFAULT|FB_QUIET);
    $joined = decode("UTF-8", $joined, FB_DEFAULT|FB_QUIET);

    my @motifs = grep { length($_) } split(/\x1F/, $joined);

    # Nettoyage BOM éventuel (très fréquent dans des .txt)
    for (@motifs) { s/^\x{FEFF}//; }

    # Motifs en lowercase
    my @m_lc = map { lc($_) } @motifs;
    ...
    my $data = decode("UTF-8", $bytes, FB_DEFAULT|FB_QUIET);

    # Tokenisation unicode : lettres/nombres + apostrophes/tirets internes
    ...
    for (my $i=0; $i<@tok; $i++) {
      ...
      if ($substring) {
        # Coréen : motif présent dans le token (자율 dans 자율주행차)
        for my $m (@motifs) {
          next unless length($m);
          if (index($t, $m) >= 0) { $matched = 1; last; }
        }
      } else {
        # Bélarussien : exact OU préfixe (pour attraper les flexions)
        for my $m (@m_lc) {
          next unless length($m);
          if ($lt eq $m || index($lt, $m) == 0) { $matched = 1; last; }
        }
      }
      ...
      for ($left,$kw,$right) { s/\t/ /g; }

      print $OUT $label, "\t", $left, "\t", $kw, "\t", $right, "\n";
    }

    close $OUT;
  ' "$txt" "$out_tsv" "$label" "$motifs_joined" "$w" "$substring"
}


generer_kwic_tsv_kr_grep() {
  local txt="$1"
  local out_tsv="$2"
  local label="$3"
  local motifs_joined="$4"  # motifs séparés par \x1F
  local w="$5"              # pas utilisé ici (on fait 1 token gauche/droite)

  : > "$out_tsv"

  local IFS=$'\x1F'
  for m in $motifs_joined; do
    [[ -z "$m" ]] && continue

    # Lignes qui contiennent le motif (match simple, compatible Mac)
    grep -F "$m" "$txt" 2>/dev/null \
    | awk -v lab="$label" -v m="$m" '
      {
        n = split($0, a, /[[:space:]]+/)
        for (i=1; i<=n; i++) {
          if (index(a[i], m) > 0) {
            ...
            kw    = a[i]
            right = (i<n ? a[i+1] : "")
            gsub(/\t/, " ", left); gsub(/\t/, " ", kw); gsub(/\t/, " ", right);
            print lab "\t" left "\t" kw "\t" right;
          }
        }
      }
    ' >> "$out_tsv"
...
generer_contextes_kwic() {
	...
	[[ -n "$motifs1" ]] && generer_kwic_tsv "$fichier_texte" "$TSV1" "$LABEL_SENS1" "$motifs1" "$CONTEXT_WORDS" "$MATCH_SUBSTRING"
	[[ -n "$motifs2" ]] && generer_kwic_tsv "$fichier_texte" "$TSV2" "$LABEL_SENS2" "$motifs2" "$CONTEXT_WORDS" "$MATCH_SUBSTRING"
