...
# ==========================================
#   Nettoyage du dump texte pour Wikipédia
# ==========================================
nettoyer_dump_wikipedia() {
  local txt="$1"

  perl -i -Mutf8 -CS -pe '
    # Supprime mots UI fréquents
    s/\bправіць\b//g;
    s/\bзыходнік\b//g;
    s/\bПераключыць змест\b//g;

    # Supprime lignes parasites typiques du haut de page (liste des langues, "xx моў", etc.)
    if (/^\s*\d+\s+моў\b/i) { $_=""; }
    if (/(العربية|Azərbaycanca|English|Español|Deutsch|Français|Русский|Українська)/) { $_=""; }

    # Supprime catégories / métadonnées de bas de page
    if (/^\s*Катэгорыі\b/i) { $_=""; }
    if (/Узята з\s+https?:/i) { $_=""; }

    # Nettoyage espaces
    s/\s{2,}/ /g;
  ' "$txt"

  # Supprime les lignes vides restantes
  sed -i '' '/^[[:space:]]*$/d' "$txt" 2>/dev/null || sed -i '/^[[:space:]]*$/d' "$txt"
}

# ==============================
#  Wikipédia : URL action=render
# ==============================
wikipedia_render_url() {
  local url="$1"

  # Match : https://xx.wikipedia.org/wiki/TITRE...
  if [[ "$url" =~ ^https?://([^/]+)\.wikipedia\.org/wiki/(.+)$ ]]; then
    local lang="${BASH_REMATCH[1]}"
    local rest="${BASH_REMATCH[2]}"

    # Enlève query/fragment si présents
    rest="${rest%%\?*}"
    rest="${rest%%\#*}"

    # Construit l’URL "render" (moins de menus, pas de skin)
    printf "https://%s.wikipedia.org/w/index.php?title=%s&action=render" "$lang" "$rest"
    return 0
  fi
...
# Type de page courant (sert à appliquer des nettoyages spécifiques)
PAGE_TYPE="generic"

# ==========================================
#  Extraction du contenu principal HTML
# ==========================================
extraire_contenu_principal_html() {
  local html_in="$1"
  local html_out="$2"
  ...
  perl -0777 -pe '
    ...
    # --- Cas Wikipédia : zone "mw-parser-output" (action=render) ---
    if (m{<div[^>]+class="[^"]*\bmw-parser-output\b[^"]*"[^>]*>.*}is) {
      $out = $&;

      # Coupe tout ce qui vient après les footers/catégories (au cas où)
      $out =~ s{<div[^>]+class="printfooter".*}{}is;
      $out =~ s{<div[^>]+id="catlinks".*}{}is;

      # IMPORTANT : garder uniquement le tout premier paragraphe de l’intro
      # => ça évite une 2e occurrence plus bas dans le lead
      if ($out =~ m{(<p\b[^>]*>.*?<\/p>)}is) {
        $out = $1;
      }

      # Retire encore quelques blocs parasites si jamais
      $out =~ s{<table\b[^>]*>.*?<\/table>}{}gis;
      $out =~ s{<div\b[^>]*>.*?<\/div>}{}gis;
    }
    else {
      # fallback : tout (sites non wiki)
      $out = $_;
    }

    $_ = "<html><head><meta charset=\"utf-8\"></head><body>$out</body></html>";
  ' "$html_in" > "$html_out"

  log_step "Extraction contenu principal → $html_out"
}
...
extraire_et_compter() {
	...
	# Nettoyage spécifique Wikipédia (évite les restes "правіць", etc.)
	if [[ "$url" =~ wikipedia\.org ]]; then
		nettoyer_dump_wikipedia "$FICHIER_TEXTE"
	fi
...
while read -r line; do
	...
	# Pour Wikipédia : on aspire la version "render"
	FETCH_URL="$line"
	if is_wikipedia_url "$line"; then
		FETCH_URL="$(wikipedia_render_url "$line")"
		log_step "Wikipédia détecté → aspiration via: $FETCH_URL"
	fi

	# Récupération du code HTML de l'URL
	CODE=$(curl -sL --max-time 20 -A "$UA" -o /dev/null -w "%{http_code}" "$FETCH_URL")
	...
	ENCODAGE=$(curl -sIL -L -A "$UA" "$FETCH_URL" | tr -d '\r' | grep -i -o 'charset=[^;[:space:]]*' | head -n1 | cut -d= -f2)
	# Si pas trouvé : "-"
	[[ -z "$ENCODAGE" ]] && ENCODAGE="-"
	# Variable "affichée" 
	...
	curl -sL --compressed -A "$UA" "$FETCH_URL" > "$FICHIER_HTML"
	...
	if [[ -s "$FICHIER_HTML" ]]; then
		...
		if [[ "$ENCODAGE" =~ [Uu][Tt][Ff]-8 ]]; then
			...
			HTML_NETTOYE="../dumps-text/${FICHIER_URLS}/clean-${n}.html"
			extraire_contenu_principal_html "$HTML_POUR_LYNX" "$HTML_NETTOYE"

			# Lynx → txt + calcule nb mots + calcule OCC + génère concordancier
			extraire_et_compter "$HTML_NETTOYE" "$n" "$line"
